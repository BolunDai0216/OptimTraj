...
class FiveLinkBiped:
    ...
    def get_mass_torque(self, batch_size=64):
        mass_list = [
        ...
        j_vec = tf.tile(_j_vec[tf.newaxis, :, :], [batch_size, 1])

        mass_torque_mat = tf.zeros([64, 0])
        for j in range(5):
            ...
            # Has shape [batch_size, state_dim]
            mass_torque_mat = tf.concat(
                [mass_torque_mat, mass_torque[:, 2:]], 1)

        # Has shape [batch_size, state_dim, 1]
        return mass_torque_mat[:, :, tf.newaxis]

    def get_angular_momentum_change(self, reference_num, reference_point):
        pass

    def get_com_derivatives(self, batch_size=64):
        # ddG1 =
        ...
        mat_zeros_1 = tf.zeros([batch_size, 1, 4])
        ...
        mat_zeros_2 = tf.zeros([batch_size, 1, 3])
        ...
        mat_zeros_3 = tf.zeros([batch_size, 1, 2])
        ...
        mat_zeros_4 = tf.zeros([batch_size, 1, 1])
        ...
        mat_zeros_5 = tf.zeros([batch_size, 1, 1])
    ...
    def update_joint_n_com_pos(self, qs, batch_size=64):
        """
        ...
        i_vec = tf.tile(_i_vec[tf.newaxis, :, :], [batch_size, 1, 1])
        j_vec = tf.tile(_j_vec[tf.newaxis, :, :], [batch_size, 1, 1])
